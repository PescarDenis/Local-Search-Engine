    ...
    def run(self, root: Path, num_processes: int = 4) -> IndexReport:
        ...
        for _ in range(num_processes):
            ...
            p = multiprocessing.Process(
                ...
                args=(paths_queue, results_queue, self._parser),
            )
            p.start()  # start it
            producers.append(p)
        ...
        def consumer_function():
            ...
            while active_producers > 0:
                parsed_file_entry = (
                    results_queue.get()
                )  # get the file entry from the result queue
                if (
                    parsed_file_entry is None
                ):  # if we hit a sentinel decrease the number of active producers
                    active_producers -= 1
                    continue
                ...
                if isinstance(parsed_file_entry, Exception):
                    ...
                    indexed_paths.add(
                        parsed_file_entry.path
                    )  # add the parsed good path into the set
                    try:
                        self._manager.process(
                            parsed_file_entry, report
                        )  # then process it to perfrom DB operations
                    except Exception:
                        report.add("error")
