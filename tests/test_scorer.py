...
def test_scorer_penalties():
    ...
    weight = scorer.score(
        path, size_bytes=3_000_000, modified_at=time.time() - 40 * 24 * 3600
    )
...
def test_scorer_path_depth_boost():
    ...
    path = Path("README.md")
    weight = scorer.score(
        path, size_bytes=1000, modified_at=time.time() - 40 * 24 * 3600
    )
...
def test_scorer_minimum_weight_floor():
    ...
    weight = scorer.score(
        path, size_bytes=5_000_000, modified_at=time.time() - 60 * 24 * 3600
    )
...
def test_scorer_medium_recency():
    ...
    weight = scorer.score(
        path, size_bytes=1000, modified_at=time.time() - 15 * 24 * 3600
    )
